...
class MoaMedMCQA(MoaBase):
    ...
    def extract_answer(self, response):
        patterns = [r'[Aa]nswer is \((.)\)', r'[Aa]nswer: \((.)\)', r'[Aa]nswer: \((.)\)', r'[Aa]nswer \((.)\)',
    ...
    def run(self):
        ...
        for i in tqdm(range(len(data_index) - 1)):
            ...
            for response_pred_dict, data_id_range_i in zip(response_pred_dict_list, data_id_range_filter):
                ...
                gt = each['gold_answer']
                is_correct = gt == response_pred_dict['pred']
...
if __name__ == '__main__':
    ...
    task = MoaMedMCQA(model, model_list_str, model_List_map, data_path, max_tokens, mode=mode, use_sc=use_sc,
                 N=N, max_process=8, sc_posi=sc_posi, ref_sample=ref_sample,
                      ppl_coef=ppl_coef, agg_max_tokens=agg_max_tokens,
                      question_bank=question_bank, dataset='MedMCQA', cache_exp=cache_exp)
    task.run()
